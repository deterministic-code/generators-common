...
export class SpecificationParser {
  ...
  parseRoutes(args: {
    ...
    const routesDoc = this.#ensureHealthRouteFirst(parse(args.routesYaml));
  ...
  async loadDatasourceTypes(idType: string): Promise<DatasourceType[]> {
    return this.parseDatasourceTypes({
  ...
  async loadViewTypes(): Promise<ViewType[]> {
    ...
    const datasourceYaml = (await reader.exists(DATASOURCE_TYPES_YAML))
      ...
      : undefined;
    return this.parseViewTypes({ viewYaml, datasourceYaml });
  }

  async loadServices(args: {
    idType: string;
    serviceClassName: (entity: string) => string;
  }): Promise<ParsedServices> {
    const reader = this.#requireReader();
    const servicesYaml = await reader.read(SERVICES_YAML);
    const related = await this.#loadRelated(args.idType);
    const routesYaml = (await reader.exists(ROUTES_YAML))
      ? await reader.read(ROUTES_YAML)
      : undefined;
    return this.parseServices({
      servicesYaml,
      views: related.views,
      datasources: related.datasources,
      routesYaml,
      serviceClassName: args.serviceClassName,
    });
  }

  async loadRoutes(args: { idType: string }): Promise<ParsedRoutes> {
    const reader = this.#requireReader();
  ...
  async load(args: {
    ...
    if (reader === undefined) {
      throw new Error("SpecificationParser.load requires a reader");
    }
    const [hasDs, hasViews, hasServices, hasRoutes] = await Promise.all([
      reader.exists(DATASOURCE_TYPES_YAML),
      reader.exists(VIEW_TYPES_YAML),
      reader.exists(SERVICES_YAML),
      reader.exists(ROUTES_YAML),
    ]);
    const [datasourceYaml, viewYaml, servicesYaml, routesYaml] =
      await Promise.all([
    ...
    const datasources =
      datasourceYaml !== undefined
        ? this.parseDatasourceTypes({ yaml: datasourceYaml, idType: args.idType })
        : [];
    const views =
      viewYaml !== undefined
    ...
    const services =
      servicesYaml !== undefined
        ? this.parseServices({
            ...
            datasources,
            routesYaml,
            serviceClassName: args.serviceClassName ?? ((entity) => entity),
          })
        : { generics: [], customs: [] };
    const routes =
    ...
    return { datasources, views, services, routes };
  }

  #requireReader(): IDeterministicReader {
    if (this.reader === undefined) {
      throw new Error("SpecificationParser requires a reader");
    }
    return this.reader;
  }
  ...
  async #loadRelated(
    idType: string,
  ): Promise<{ datasources: DatasourceType[]; views: ViewType[] }> {
    const reader = this.#requireReader();
    ...
    const datasources =
      datasourceYaml !== undefined
        ...
        : [];
    const views = hasViews ? await this.loadViewTypes() : [];
    return { datasources, views };
  }
